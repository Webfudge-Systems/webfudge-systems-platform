...
export default function WorkspaceCalendarPage() {
  ...
  const onEventClick = useCallback(
    ({ kind, entity }) => {
      ...
      if (kind === 'meeting' && entity?.id) {
        window.location.href = `${crmBase}/meetings/${entity.id}`;
        return;
      }
      if (kind === 'task' && entity?.id) {
        router.push(`/tasks/${entity.id}`);
        return;
      }
      if (kind === 'project' && entity?.slug) {
        router.push(`/projects/${entity.slug}`);
      }
    },
    [router]
  );
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      <UnifiedWorkspaceCalendar events={fcEvents} onDatesSet={handleDatesSet} onEventClick={onEventClick} />
    </div>
  );
}
