  ...
  return (
    ...
    >
      <div className={`flex items-center justify-between rounded-t-2xl border-b px-4 py-3 ${style.header}`}>
        ...
        <span className={`rounded-full px-2.5 py-0.5 text-xs font-bold ${style.badge}`}>{tasks.length}</span>
      </div>
      <div className="flex flex-1 flex-col gap-2.5 overflow-y-auto p-3">
        {tasks.length === 0 ? (
          ...
          >
            <p className="text-[11px] text-gray-400">{isOver ? 'Release to move here' : 'No tasks'}</p>
          </div>
        ) : (
...
export function MyTasksKanbanBoard({ tasks, router, updateTask, activeTab }) {
  ...
  const filteredStages = useMemo(() => {
    if (activeTab === 'all' || activeTab === 'OVERDUE') return stagesData;
    ...
    >
      <div className="flex gap-4 overflow-x-auto p-4 pb-5 md:p-5">
        {filteredStages.map(({ key, label, tasks: colTasks }) => (
          <KanbanColumn key={key} stageKey={key} label={label} tasks={colTasks} isOver={overId === key} router={router} />
        ))}
      </div>
      ...
      {grouped.map((section) => (
        <section key={section.key} className="px-4 py-5 md:px-6">
          <div className="mb-3 flex items-center gap-2">
            <h2 className="text-xs font-extrabold uppercase tracking-widest text-gray-500">{section.label}</h2>
            <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-bold text-gray-700">{section.tasks.length}</span>
          </div>
          <ul className="space-y-2">
            {section.tasks.map((row) => {
              ...
              return (
                <li
                  key={row.id}
                  className="flex flex-col gap-3 rounded-xl border border-gray-100 bg-gray-50/50 p-4 transition hover:border-orange-100 hover:bg-white sm:flex-row sm:items-center sm:gap-4"
                >
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-semibold text-gray-900 hover:text-orange-600">{row.name}</p>
                  ...
                  <div className="flex flex-wrap items-center gap-2 sm:justify-end">
                    {row.project ? (
                      <button
                        ...
                        className="inline-flex max-w-[200px] items-center gap-1.5 rounded-lg border border-orange-200 bg-orange-50 px-2.5 py-1.5 text-xs font-semibold text-orange-900 shadow-sm hover:border-orange-300"
                      >
                    ...
                    <div className={`text-right text-xs ${overdue ? 'text-red-700' : 'text-gray-600'}`}>
                      <p className="font-semibold">{row.dueDate ? formatShortDate(row.dueDate) : '—'}</p>
...
export function MyTasksTimelineView({ tasks, router }) {
  ...
  if (!range) {
    return (
      <div className="space-y-4 p-6">
        <p className="text-sm text-gray-600">No start or due dates on these tasks. Add dates on task detail to see the timeline.</p>
        <ul className="space-y-2">
          {unscheduled.map((t) => (
            <li key={t.id}>
              <Button variant="ghost" className="h-auto justify-start px-2 py-1 text-left font-medium" onClick={() => router.push(`/tasks/${t.id}`)}>
                {t.name}
              </Button>
            </li>
          ))}
  ...
  const ticks = Array.from({ length: tickCount }, (_, i) => {
    ...
    return { ts, label: new Date(ts).toLocaleDateString('en-US', { month: 'short', day: 'numeric' }) };
  });
    ...
    <div className="overflow-x-auto p-4 md:p-6">
        ...
        {rows.map(({ task: t, startMs, endMs }) => {
          ...
          return (
            <div key={t.id} className="flex items-center gap-3 border-b border-gray-50 py-2.5 last:border-0">
              ...
              <div className="relative h-8 flex-1 rounded-md bg-gray-100/80" style={{ minWidth: trackMinWidth }}>
                <button
                  type="button"
                  onClick={() => router.push(`/tasks/${t.id}`)}
                  className={`absolute top-1/2 h-5 -translate-y-1/2 rounded-md shadow-sm transition hover:opacity-95 ${
                    overdue ? 'bg-gradient-to-r from-red-400 to-red-500' : 'bg-gradient-to-r from-orange-400 to-pink-400'
                  }`}
      ...
      {unscheduled.length > 0 ? (
        <div className="mt-8 border-t border-gray-200 pt-5">
          <p className="mb-2 text-xs font-bold uppercase tracking-wide text-gray-500">No schedule</p>
          <ul className="flex flex-wrap gap-2">
            {unscheduled.map((t) => (
              <li key={t.id}>
                <Button variant="outline" size="sm" className="text-xs font-medium" onClick={() => router.push(`/tasks/${t.id}`)}>
                  {t.name}
                </Button>
              </li>
            ))}
