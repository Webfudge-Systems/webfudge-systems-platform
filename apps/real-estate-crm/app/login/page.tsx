  ...
  return (
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-brand-primary to-orange-600 relative flex-col justify-center px-16 py-20">
        <LoginBrandCorner
          brandIconPath={RE_SITE.brandIconPath}
          brandName={RE_SITE.brandName}
        />
        <div className="max-w-lg">
          <LoginProductCredit productName={RE_SITE.name} creatorLine={RE_SITE.brandName} />
          <h1 className="text-5xl font-bold text-white mb-6">Welcome back</h1>
          <p className="text-xl text-white/90 mb-4">{RE_SITE.loginTagline}</p>
          <p className="text-white/80 leading-relaxed">{RE_SITE.loginDetail}</p>
          <div className="mt-12 grid grid-cols-3 gap-6">
            {RE_SITE.loginFeatures.map((item) => (
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center p-8 lg:p-16">
        <div className="w-full max-w-md mx-auto">
          <LoginMobileBrandHeader
            ...
            creatorLine={RE_SITE.brandName}
          />
          <h2 className="text-3xl font-semibold text-brand-dark mb-2">Sign in</h2>
          <p className="text-gray-600 mb-8">Enter your credentials to access the workspace.</p>
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <Input id="email" name="email" type="email" autoComplete="email" value={email} onChange={(e: any) => setEmail(e.target.value)} placeholder="you@company.com" error={errors.email} className="w-full" />
              {errors.email && <p className="mt-1 text-sm text-red-600">{errors.email}</p>}
            </div>
            <div>
              <label htmlFor="password" className="block text-sm font-medium text-brand-dark mb-1.5">Password</label>
              <div className="relative">
                <Input id="password" name="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={(e: any) => setPassword(e.target.value)} placeholder="••••••••" error={errors.password} className="w-full pr-10" />
