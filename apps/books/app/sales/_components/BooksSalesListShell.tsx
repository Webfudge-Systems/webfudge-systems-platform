...
type Props<T extends Record<string, any>> = {
  ...
  /** KPI cards shown above tabs. Keep CRM-like 4 cards. */
  kpis: Array<{
    title: string
    value: number | string
    subtitle?: string
  ...
  /** Optional tabs like CRM deals stage tabs. */
  tabs?: TabItem[]
  activeTab?: string
...
export default function BooksSalesListShell<T extends Record<string, any>>({
  title,
  subtitle,
  kpis,
  tabs,
  ...
  // Keep search UI consistent with CRM; actual filtering can be added per page later.
  ...
  const exportCsv = () => {
    ...
    a.href = url
    a.download = `books-sales-${title.toLowerCase().replace(/\s+/g, '-')}.csv`
    ...
    <div className="space-y-6 bg-white min-h-full">
      ...
      <div className="relative" ref={toolbarRef}>
        <TabsWithActions
          ...
          // Match CRM list bar: always show the (+) icon; if no create route yet, keep it as a safe no-op.
          showAdd={true}
