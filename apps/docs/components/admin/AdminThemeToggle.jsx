...
/** Animated sun/moon toggle styled for the blue admin shell. */
function SolarSwitch({ isDark }) {
  ...
  return (
    <motion.div animate={isDark ? 'checked' : 'unchecked'} className="flex">
...
export default function AdminThemeToggle({ className = '' }) {
  ...
  return (
    <button
      ...
      className={`inline-flex h-9 w-9 items-center justify-center rounded-xl border transition-colors ${className}`}
      style={{
    ...
    >
      {mounted ? <SolarSwitch isDark={isDark} /> : <span className="h-[18px] w-[18px]" />}
    </button>
  );
}
