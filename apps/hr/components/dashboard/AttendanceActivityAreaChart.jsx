...
import { buildAttendanceActivitySeries } from '../../lib/attendanceActivitySeries'
...
export default function AttendanceActivityAreaChart({
  activeEmployeeCount,
  days = 7,
  className = '',
}) {
  ...
  const { points, max } = useMemo(
    () => buildAttendanceActivitySeries(activeEmployeeCount, days),
    [activeEmployeeCount, days]
  )

  const checkedInValues = points.map((p) => p.checkedIn)
  const stackedTop = points.map((p) => p.checkedIn + p.onLeave)
  const totalValues = points.map((p) => p.total)

  const checkedInBand = buildBandPath(
    points.map(() => 0),
    checkedInValues,
    max
  )
  const onLeaveBand = buildBandPath(checkedInValues, stackedTop, max)
  ...
  const gradCheckedIn = `attendance-area-checkedin-${uid}`
  const gradOnLeave = `attendance-area-onleave-${uid}`
  ...
  return (
    <div className={className}>
      ...
      <svg
        ...
        role="img"
        aria-label={`Attendance activity over the last ${days} days`}
      >
        <defs>
        ...
        {hasActivity ? (
          <>
            <path d={checkedInBand} fill={`url(#${gradCheckedIn})`} />
            <path d={onLeaveBand} fill={`url(#${gradOnLeave})`} />
            <path
              d={totalLine}
              fill="none"
              stroke="#FF7A00"
            ...
            {points.map((p, i) => {
              ...
              return (
                <circle
                  ...
                  strokeWidth="1"
                >
                  <title>{`${p.shortLabel}: ${p.checkedIn} checked in, ${p.onLeave} on leave`}</title>
                </circle>
              )
            })}
          </>
