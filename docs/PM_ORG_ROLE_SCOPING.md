...
- **Member** (any other org role, including the default **Member** template): **only projects** where they are project manager or **team member**; **no project create** or **project settings** edits; **tasks** limited to visibility rules below; **task updates**: full edit on tasks where they are **assignee or collaborator**; **status only** on other visible tasks; **may create tasks** on projects they belong to (assignees pending approval) and **subtasks** on tasks assigned to them; **no task delete** (delete UI hidden).
