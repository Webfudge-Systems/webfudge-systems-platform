...
| **[PM_ORG_ROLE_SCOPING.md](./PM_ORG_ROLE_SCOPING.md)**                                                   | PM row-level rules by **org** role: managers see all projects but edit assigned only; members scoped to assigned projects/tasks; member task updates = progress + status only   |
