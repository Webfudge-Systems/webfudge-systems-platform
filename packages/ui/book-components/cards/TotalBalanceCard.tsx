...
export function TotalBalanceCard({
  ...
  const displayBalanceLabel = useMemo(() => {
    const w = wallets.find((x) => x.code === resolvedCurrencyCode)
    if (w) return w.balanceLabel
    return balanceLabel
  }, [wallets, resolvedCurrencyCode, balanceLabel])
  ...
  return (
    ...
    >
      <div className="flex min-h-0 flex-1 flex-col space-y-6 overflow-auto p-6 md:p-7">
        ...
        <div>
          <p className="text-3xl font-bold tracking-tight text-[var(--books-text-primary,#1a1a1a)] sm:text-[2.35rem]">
            {displayBalanceLabel}
          </p>
          {trendLabel ? (
            <p
